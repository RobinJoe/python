...
def checklinks(infile, count, redirects, total, verbose):
    ...
    for ulink in soup('ulink'):
        ...
        try:
            if redirects: status = requests.get(url).status_code
            else: status = requests.head(url).status_code
            ...
            #print(WARNING + "Warning: " + ENDCOLOR + str(e))
            pass
    if count:
        if badcount > 0:
            print(HEADER + path.basename(infile) + ENDCOLOR)
...
def main():
    ...
    parser.add_argument('-r', '--redirects', action='store_true', default=False, help='Follow 301 redirects and treat redirect target as link. SLOW.')
    parser.add_argument('-t', '--total', action='store_true', default=False, help='Print only the total number of invalid links')
