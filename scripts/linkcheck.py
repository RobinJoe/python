...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-r', '--redirects', action='store_true', default=False,
    ...
    if path.isfile(args.INPUT):
        badtotal = checklinks(args.INPUT, args.count, args.redirects,
                              args.total, args.verbose)
    else:
        for filename in iglob(args.INPUT + '*.xml'):
            badtotal += checklinks(filename, args.count, args.redirects,
                                   args.total, args.verbose)
